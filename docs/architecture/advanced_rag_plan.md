...
- 当前仓库状态（按 2026-03-17 代码快照）已不止停留在 M0/M1 设计阶段，实际落点更接近：`M1 收尾 + M2 主链已落地 + M3/M4 起步`。
- 主链路能力：已从单路向量检索升级到 `lexical + dense + fusion + rerank` 的混合检索主链，并具备基础 query rewrite、多查询召回、query route、证据拒答与离线评测能力，但索引修复、门禁与精细治理仍未完全闭环。
- 已完成能力（M0）：
  - 文档清洗与 chunk 元数据基线。
  - PDF 页级 OCR 触发与 native/OCR 融合。
  - OCR 阈值可配置，基础测试已覆盖。
- 已落地但尚未闭环的能力：
  - query normalize、query variants、模型驱动的查询改写与多查询召回。
  - `dense + lexical + fusion + rerank + context pack` 检索主链。
  - 基于历史/书外实时问题的初版查询路由。
  - 证据约束回答、证据不足拒答、基础 groundedness 校验。
  - `rag_eval` 离线评测 CLI、固定测试数据与一键脚本。
- 主要缺口：
  - 查询前增强仍未完成术语治理与 query route 深化（改写、多查询开关已独立化，但策略仍偏轻量）。
  - 分块重建仍以整书替换为主，增量重建、索引修复与幂等重跑仍需补齐。
  - 检索中精排与恢复策略仍需继续调优（主链已形成，但门限和容量治理仍偏保守）。
  - 生成后校验仍偏启发式（证据约束、拒答开关已存在，但一致性校验未闭环）。
  - 评测与发布门禁不足（已有离线评测基础设施，但尚未接入 CI 阈值阻断与版本对比闭环）。
...
### 5.2 检索中（Retrieval）
- 混合检索：`dense(Qdrant) + lexical(OpenSearch BM25)` 并行召回，融合打分。
...
当前判断：
- 已具备 query normalize、query variants、模型改写、多查询召回、独立策略开关与初版 query route。
- 尚缺术语归一映射、metadata filter 深用、增量重建与参数化治理。

### M2：检索中增强（主链已落地，持续调优中）
目标：从“召回到”升级为“召回准”。

关键交付：
- 混合检索（Qdrant dense + OpenSearch BM25）主链上线。
- 两阶段检索（TopN + rerank）。
- 去重与上下文打包策略。
...
当前判断：
- `Qdrant dense + OpenSearch BM25 + fusion + rerank + context pack` 已在 chat 主链落地。
- 当前重点从“有没有主链”转为“参数、容量、降级与恢复策略是否足够稳”。

### M3：检索后与生成约束（初版已落地，未闭环）
目标：降低幻觉，提高可解释性。
...
当前判断：
- 证据约束 prompt、证据不足拒答、基础 groundedness 校验已在 chat 链路中落地。
- 当前一致性校验仍是轻量启发式，尚未形成更强的 answer-citation alignment / 低置信度降级闭环。

### M4：评测与发布门禁（基础设施已落地，门禁未闭环）
...
当前判断：
- `rag_eval` CLI、固定测试数据、报告输出与一键脚本已具备。
- 当前 CI 仅覆盖 OpenAPI / Go / 前端基础校验，尚未把 RAG 指标阈值接入主分支门禁。
...
## 13. 下一步（建议立即执行）
- S1：补齐增量重建 / 索引修复链路，避免分块与索引治理长期停留在整书替换模式。
- S2：把 `rag_eval` 指标阈值接入 CI，形成最小可用的发布门禁。
- S3：继续补强术语归一、metadata filter 与 query route 策略深度。
